...
def process_a_single_sample(k,each_point,inputs,all_clusters_img=None):
    # global score_arr
    ...
    if darray[int(centroid[1]),int(centroid[0])]:
        for index,rectangle_pixels in enumerate(rectangle_pixels_list):
            ...
            if gdi is not None and gdi_plus is not None:
                ...
                result.append([cx,cy,score])
                print(k,index,'valid\n')
                gdi2.draw_refined_pose(copy.deepcopy(inputs['image']),path='temp/poses/{0}_{1}.png'.format(k,index))
            else:
                result.append([cx,cy,0])
                print(k,index,'\n')
            ...
            cv2.imwrite(dump_dir+'/bmaps/bmap{0}_{1}_denoised.jpg'.format(k,index),bmap_denoised)
...
def generate_graspability_scores(inputs,centroid_pixels):
    ...
    # score_arr = np.zeros(darray.shape)
    ...
    return copy.deepcopy(score_arr)