    ...
    if darray[int(centroid[1]),int(centroid[0])]:
        for index,rectangle_pixels in enumerate(rectangle_pixels_list):
            ...
            if gdi is not None and gdi_plus is not None:
                ...
                result.append([cx,cy,score,grasp_width,centroid[0],centroid[1]])
                print(k,index,'valid\n')
                ...
                result.append([cx,cy,0,0,centroid[0],centroid[1]])
                print(k,index,'\n')
...
def generate_graspability_scores(inputs,centroid_pixels):
    ...
    h,w = darray.shape
    angle_arr = -1*np.ones((h,w,inputs['num_dirs']))
    ...
    for k in range(N):
        ...
        for index in range(result.shape[0]):
            ...
            # if score_arr[j,i] < 0:
            #     score_arr[j,i] = 0
            # score_arr[j,i] = sample[2]+score_arr[j,i]/3
            # angle_arr[j,i] = sample[3]
            # width_arr[j,i] = max(sample[4],width_arr[j,i])
            angle_arr[j,i,index] = sample[2]
            angle_arr[y,x,index] = sample[2]
            angle_arr[y1,x1,index] = sample[2]
            if score_arr[j,i] < sample[2]:
                score_arr[j,i] = sample[2]
                width_arr[j,i] = sample[3]
                csize = int(score_arr[j,i]/20) + 1
                cv2.circle(image, (i, j), csize, (0,0,0), -1)
            if score_arr[y,x] < sample[2]/2:
                score_arr[y,x] = sample[2]/2
                # width_arr[y,x] = sample[3]
                csize = int(score_arr[y,x]/20) + 1
                cv2.circle(image, (x, y), csize, (0,0,0), -1)
            if score_arr[y1,x1] < 3*sample[2]/4:
                score_arr[y1,x1] = 3*sample[2]/4
                # width_arr[y1,x1] = sample[3]
                csize = int(score_arr[y1,x1]/20) + 1
                cv2.circle(image, (x1, y1), csize, (0,0,0), -1)
