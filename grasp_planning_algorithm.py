...
def median_depth_based_filtering(darray,median_depth_map,img):
    ...
    for i in range(w):
        for j in range(h):
            if mask[j][i] and np.random.random()>0.9:#0.9:
...
def run_grasp_algo(img,darray,path,final_attempt=True):
    # global index
    # st = time.time()
    ...
    clustter_img_orig = copy.deepcopy(img)
    ...
    centroid_pixels_3D = median_depth_based_filtering(darray,median_depth_map,img.copy())
    ...
    for level in range(num_of_clusters,num_of_clusters+span):
        ...
        rectangle_img = copy.deepcopy(rectangle_img_orig)
        axis_img = copy.deepcopy(axis_img_orig)    
        try:
            # start_time = time.time()
            ...
            return [],False,(0,0),None, False,None,None
        ...
        #     directions = 1
            # gripper_width = 15
            # pixel_finger_width = 4

        # st = time.time()
        for k in range(len(centers)):
            cluster = np.array(centroid_pixels[label.ravel() == k, :], np.int32)
            # # print cluster
            # points = np.zeros([2 * len(cluster), 2], dtype=np.int32)
            # all_points = np.zeros([])
            # for j in range(len(cluster)):
            #     points[2 * j, :] = [cluster[j, 0], cluster[j, 1]]
            #     points[2 * j + 1, :] = [cluster[j, 4], cluster[j, 5]]
            ...
            green_part = int((k*50)%255)
            blue_part = int((k*100)%255)
            red_part = int((k*150)%255)
            cv2.circle(clustter_img, (int(centers[k, 0]), int(centers[k, 1])), 5, (255, 0, 0), -1)
            cv2.circle(clustter_img, (int(centers[k, 0]), int(centers[k, 1])), 3, (blue_part,green_part,red_part), -1)
            ...
            if darray[int(centroid[1]),int(centroid[0])]:
                for index,rectangle_pixels in enumerate(rectangle_pixels_list):
                    ...
                    np.savetxt(path+'/rectangle_{0}_{1}.txt'.format(k,index),rectangle_pixels)
                    np.savetxt(path+'/angle_{0}_{1}.txt'.format(k,index),[angle_list[index]-radians(180)])
                    cv2.imwrite(path+'/bmap{0}_{1}.jpg'.format(k,index),bmap)#.astype(np.uint8))
                    cv2.imwrite(path+'/bmap{0}_{1}_denoised.jpg'.format(k,index),bmap_denoised)#.astype(np.uint8))
    ...
    if len(GDI)==0 :
        ...
        if not final_attempt:
            ...
            return None,False,None,False,False, None, None
        else:
    ...
    return target,True,np.array([cy,cx]),valid_flag,boundary_pose, min_depth_difference, fov, final_pose_rect_img
...
def grasp_planning(mp):
    ...
    image = mp.cur_image
    dmap = mp.cur_dmap.astype(np.float64)/1000
    pc_arr = mp.cur_pc
    ...
    for attempt_num in range(total_attempt):
        if attempt_num == 2:
            final_attempt = True
        st = time.time()
        mp.action,flag,center,valid, boundary_pose, min_depth_difference, mp.fov_points, final_img = run_grasp_algo(image.copy(),dmap.copy(),path,final_attempt=final_attempt)
