...
from grasp_planning_algorithm_new import run_grasp_algo,grasp_planning
import os
import threading
...
class ur10_grasp:
	...
	def main(self):
		...
		while True:
			...
			self.gripper.run(self.gripper_closing-10)
			time.sleep(1)
	...
	def move_the_robot_new_way(self,wpose,loop=True):
		ee_pose = self.arm.get_current_pose(self.end_effector_link).pose
		wpose.position.x -= 0.015 # observed error correction
		wpose.position.y += 0.015 # observed error correction
