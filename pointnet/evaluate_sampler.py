...
from utils_gs import Parameters, draw_top_N_points
from utils_gs import points_to_pixels_projection
from utils_gs import select_top_N_grasping_points_via_top_points_method
...
# samplers = ['votenet', 'pointnet2_fcn']

# samplers = ['random','baseline','baseline_adaptive','fcn_rgb','fcn_rgbd','fcn_depth', 'pointnet2_pcd','votenet', 'pointnet2_fcn','votenet', 'pointnet2_fcn']
...
for sampler in samplers:
	...
	if sampler == 'transformer' or sampler == 'transformer_pcd':
		...
		checkpoint = torch.load('log/{0}/checkpoint_mid.tar'.format(sampler,data_type))
	...
	if sampler == 'fcn_rgb' or sampler == 'fcn_depth' or sampler == 'fcn_rgbd':
		...
		checkpoint = torch.load('../FCN/mid_{1}/weights.torch'.format(data_type,sampler))
	...
	if sampler == 'pointnet2_pcd' or sampler == 'pointnet2_pcd_rgb' or sampler == 'votenet':
		...
		device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
		net.to(device)
		CHECKPOINT_PATH = 'log/checkpoint_{0}_{1}.tar'.format('mid',sampler)
	...
	if sample_type == 'sim':
		data_path = '../../data/data_{0}/val'.format(data_type)
		...
		if data_type == 'high':
			...
			data_path = '../real_data/test_data_mid_level_S'
	...
	for i in range(start_sample,start_sample+total_samples):
		...
		if sampler == 'transformer' or sampler == 'transformer_pcd':
			point_cloud = np.load(sample_path+'_pc_complete.npy').reshape(-1,3)#['pc'] # Nx6
			...
			if sample_type == 'sim':
				...
				bin_floor_filter = np.load(sample_path+'_filter_mask.npy')
			...
			# top_grasp_points[:,1] = temp
			if args.use_color:
				color_cloud = cv2.imread(sample_path+'_ref_image.png').reshape(-1,3)
		...
		if sampler == 'random':
			...
			top_grasp_points = points_to_pixels_projection(points,w=param.w,h=param.h,fx=param.f_x,fy=param.f_y)
		...
		if sample_type == 'real':
			draw_top_N_points(top_grasp_points,image)
