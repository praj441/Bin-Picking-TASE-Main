...
from grasp_net import ClusterNet
...
sys.path.append(os.path.join(ROOT_DIR, 'grasp_data'))
from pose_data import PoseDataSet, collate_fn
data_root = "grasp_data/AppleDataSet"
annotations_json = data_root+"/pose_annotations.json"
dataset = PoseDataSet(data_root, annotations_json, augmentation_prob=0, num_points=5000)
BATCH_SIZE = 4
# DATLEN=2
dataset_test = PoseDataSet(data_root, annotations_json)
split_idx = round(0.8*len(dataset))
split_idx -= split_idx%BATCH_SIZE
split_idx = len(dataset)-split_idx
indices = torch.randperm(len(dataset)).tolist()
dataset = torch.utils.data.Subset(dataset, indices[:-split_idx])
dataset_test = torch.utils.data.Subset(dataset_test, indices[-split_idx:])
TRAIN_DATALOADER = torch.utils.data.DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=2, collate_fn=collate_fn)
TEST_DATALOADER = torch.utils.data.DataLoader(dataset_test, batch_size=BATCH_SIZE, shuffle=True, num_workers=2,collate_fn=collate_fn)
...
def train_one_epoch():
	...
	for batch_idx, (inputs, targets) in enumerate(TRAIN_DATALOADER):
		...
		assert True not in torch.isnan(inputs)
		assert True not in torch.isnan(targets)
		# Forward pass
		optimizer.zero_grad()
		# print('size',batch_data_label['point_clouds'].shape)
		inputs = {'point_clouds': inputs}
		end_points = net(inputs)
		
		end_points['obj_cloud'] = inputs
		end_points['pose_gt'] = targets
		# Compute loss and gradients, update parameters.
		# for key in batch_data_label:
		# 	# print(key)
		# 	assert(key not in end_points)
		# 	end_points[key] = batch_data_label[key]
		loss = criterion(end_points)#, DATASET_CONFIG)
		print('loss:',loss)
...
def evaluate_one_epoch():
	...
	for batch_idx, (inputs, targets) in enumerate(TEST_DATALOADER):
		...
		# Forward pass
		inputs = {'point_clouds': batch_data_label['obj_cloud']}
		with torch.no_grad():
			end_points = net(inputs)
