...
from loss_helper import get_loss_transformer, computer_ab_acc, compute_gqs_acc
...
BN_DECAY_STEP = 20
BN_DECAY_RATE = 0.5
LR_DECAY_STEPS = [int(x) for x in [80,120,160]]
LR_DECAY_RATES = [float(x) for x in [0.1,0.1,0.1]]
...
def get_parser():
	...
	parser.add_argument('--config', type=str, default='Stratified_Transformer/config/bin/transformer_pcd.yaml', help='config file')
...
save_folder = args.weight_folder
CHECKPOINT_PATH = os.path.join(LOG_DIR, save_folder+'/checkpoint.tar')
...
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=BATCH_SIZE,
	shuffle=True, num_workers=2, worker_init_fn=my_worker_init_fn, collate_fn=partial(collate_fn_limit, max_batch_points=args.max_batch_points, logger=None))
...
net.to(device)
criterion = get_loss_transformer
...
def train_one_epoch():
	...
	avg_loss = 0.0
	avg_loss_vote = 0.0
	for batch_idx, (coord, feat, target, offset) in enumerate(TRAIN_DATALOADER):
		...
		end_points = {}
		end_points['gqs_score'] = output
		end_points['seed_xyz'] = None
		end_points['gqs_target'] = target
		...
		print(batch_idx,'gqs_loss:',end_points['gqs_loss'].item())
		...
		for key in end_points:
			if 'gqs_loss' in key or 'acc' in key or 'ratio' in key:
...
def evaluate_one_epoch():
	...
	for batch_idx, (coord, feat, target, offset) in enumerate(TEST_DATALOADER):
...
def train(start_epoch):
	...
	for epoch in range(start_epoch, MAX_EPOCH):
		...
		# ********* plots *******************
		torch.save(save_dict, os.path.join(LOG_DIR, save_folder+ '/checkpoint.tar'))
		torch.save(save_dict, os.path.join(LOG_DIR, save_folder+'/checkpoint{0}.tar'.format(epoch)))
		np.savetxt(LOG_DIR+'/'+save_folder+'/train_loss_gqs.txt',loss_list_train_gqs,fmt='%1.4f')
		# np.savetxt(LOG_DIR+'/'+save_folder+'/train_loss_vote.txt',loss_list_train_vote,fmt='%1.4f')
		np.savetxt(LOG_DIR+'/'+save_folder+'/val_loss_gqs.txt',loss_list_gqs,fmt='%1.4f')
		# np.savetxt(LOG_DIR+'/'+save_folder+'/val_loss_vote.txt',loss_list_vote,fmt='%1.4f')
		...
		# plt.plot(loss_list_vote)
		# plt.savefig(os.path.join(LOG_DIR,save_folder+ 'loss_vote.png'))
		# plt.clf()

		plt.plot(loss_list_gqs)
		plt.plot(loss_list_train_gqs,color='red')
		plt.savefig(os.path.join(LOG_DIR,save_folder+ '/loss_gqs.png'))
		plt.clf()

		# plt.plot(loss_list_angle)
		# plt.savefig(os.path.join(LOG_DIR,save_folder+ 'loss_angle.png'))
		# plt.clf()

		# plt.plot(loss_list_width)
		# plt.savefig(os.path.join(LOG_DIR,save_folder+ 'loss_width.png'))
		# plt.clf()
		#************ plots done **********************
