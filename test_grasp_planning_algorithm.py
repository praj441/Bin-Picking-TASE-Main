...
def median_depth_based_filtering(darray,median_depth_map):
	...
	for i in range(w):
		for j in range(h):
			if mask[j][i] and np.random.random()>0.9:
				filtered.append([i,j])
...
def run_grasp_algo(img,darray,path,final_attempt=True):
	...
	# darray_empty_bin = cv2.resize(darray_empty_bin,(w,h))
	...
	depth_image = cv2.imread('result_dir/samples/4/depth_image.jpg')
	...
	filtered_pixels,centroid_pixels = send_request_for_pixel_filtering(pixels_list,darray)  # Service Call for Filtered Pixels #
	print('time in old sampling',time.time()-st,'#samples',centroid_pixels.shape)

	st = time.time()
	centroid_pixels = median_depth_based_filtering(darray,median_depth_map)
	print('time in new sampling',time.time()-st,'#samples',centroid_pixels.shape)
	...
	centroid_pixels = np.float32(centroid_pixels)
	...
	for level in range(num_of_clusters,num_of_clusters+span):
		...
		try:
			# start_time = time.time()
			distances_of_centers_from_datapoints, label, centers = cv2.kmeans(centroid_pixels, K=level,
																		  criteria=criteria,
																		  attempts=6,
...
if __name__ == "__main__":
	...
	sample_path = 'result_dir/samples/5'
	...
	image = cv2.imread(sample_path+'/ref_image.png')
	darray = np.loadtxt(sample_path+'/depth_array.txt')
